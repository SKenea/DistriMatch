...
availability.js    - Signaux de dispo sur la fiche (UC11, EPIC-T2) : statut de chaque aliment (« Dispo / Pas dispo / Pas d'info ») et puce d'etat machine a droite du nom (« Fonctionne / Vide / En panne / Pas d'info »), via `resolveProductStatus` / `resolveMachineStatus` (utils.js, pures). On signale en touchant l'aliment (« Il y en a / Plus rien ») ou la puce ; plus de fenetre « Il reste quoi ? ». QR `&confirm=1` -> liste mise en avant (`focusSignalFromQr`)
...
| UC11 | Signal de fraicheur | Signal de dispo par produit (vu dispo / vu absent) ou machine (vide / en panne / fonctionne), en un tap | **non** (exception assumee) | Supabase RPC `confirm_availability` (migration 007) : rate limit par appareil, poids 0.5 anonyme / 0.8 connecte, aucune ecriture directe dans la table |
...
- Le stylo « Modifier » est visible sur toute fiche (plus seulement depuis Favoris) ; la connexion est verifiee au clic (UC2). En edition, le bouton d'un produit dit « Vendu ici / Plus vendu » (le catalogue), jamais la dispo du moment.
